...
<script>
  // Data produk dikirim dari PHP (hasil query database) ke JavaScript,
  // supaya fungsi keranjang (tambahKeKeranjang, dst) tetap bisa mencari produk by id
  // di halaman manapun (Beranda, Produk, Keranjang, dll).
  ...
  // Data diskon aktif dikirim ke JS agar bisa divalidasi & dihitung saat checkout
  ...
  // Data customer yang login, untuk auto-isi form checkout
  const customerLogin = <?= $customerLogin ? json_encode($customerLogin, JSON_UNESCAPED_SLASHES) : 'null' ?>;

  // Data user yang login (siapa pun rolenya), dipakai untuk cek status login sebelum checkout
